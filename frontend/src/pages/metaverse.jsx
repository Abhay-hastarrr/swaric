...
const Metaverse = () => {
	// Move these inside the component to fix HMR issues
	...
	const connectToSocket = (scene) => {
		try {
			socketRef.current = io.connect(server, { secure: true, query: { name: localStorage.getItem("name") } });
		} catch (e) {
			console.log(e);
		}
		...
		socketRef.current.on('connect', () => {
				...
				try {
					for (const socketId in AllSockets) {
						...
						if (user.socketId !== socketIdRef.current && !phaserPlayers[user.socketId]) {
							let newPlayer = createNewPlayer(scene, user, phaserPlayers, otherPlayers);
			...
			socketRef.current.on('player-move', (data) => playerMove(data, phaserPlayers, otherPlayers));
			...
			socketRef.current.on('user-disconnected', (socketId) => userDisconnected(socketId, phaserPlayers, otherPlayers));
		});
	};
